...
public class QuestionsBucket {
    ...
    public void questionsInit() {
        ...
        addNewQuestion("What is a design patterns?", "Finished design that can be transformed directly into code", "General repeatable solution to a commonly occurring problem", "An abstract type that is used to specify a behavior that classes must implement", "None of the above", 2);
        ...
        addNewQuestion("Which of the following options better describe the Primitive types?", "Data type that do not have any functionality other than simple data strorage", "Data types that reference objects", "Data types that are not built into the Java language", "None of the above", 1);
        addNewQuestion("When should we use Wrappers over Primitive types?", "As an argument of methods that expect and object", "To use object methods (e.g. convert from and to String)", "All object contains a reference to a corresponding class object", "All of the above", 4);
